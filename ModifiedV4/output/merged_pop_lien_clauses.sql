...
IS
    ...
    CURSOR clause IS
        SELECT  cls_heading, 
                ...
                sbcl_cls_code, 
                cls_type,
                DECODE (cls_type, 'CL', 'Clause', 'WR', 'Warranty', 'SC', 'Special Conditions') type_desc,
                clp_pro_code, 
                clp_pro_sht_desc, 
                clp_scl_code,
                cls_editable
    ...
    CURSOR pckge_clauses IS
        ...
        WHERE   pro_code = clp_pro_code(+)
        AND     sbcl_cls_code NOT IN (SELECT plcl_sbcl_cls_code
                                     FROM   gin_policy_lvl_clauses
                                     WHERE  plcl_pol_batch_no = v_pol_batch_no)
    ...
    CURSOR clause2 IS
        ...
        AND     cls_code NOT IN (SELECT plcl_sbcl_cls_code
                                FROM   gin_policy_lvl_clauses
                                WHERE  plcl_pol_batch_no = v_pol_batch_no)
        AND     NOT EXISTS (SELECT poscl_scl_code, poscl_cls_code
...
BEGIN
    ...
    IF NVL (v_pro_mult_class, 'N') = 'N' THEN
        FOR cls IN clause
        LOOP
            v_rownum := v_rownum + 1;
    ...
    ELSE
        ...
        LOOP
            IF  NVL (pcls.sclcnt, 0) = NVL (pcls.clscnt, 0) AND NVL (pcls.clscnt, 0) != 0 THEN
                INSERT INTO gin_policy_lvl_clauses (
                    plcl_sbcl_cls_code, 
                    plcl_sbcl_scl_code,
                    plcl_pro_sht_desc, 
                ...
                IF NVL (pcls.cls_editable, 'N') = 'Y' THEN
                    SELECT cls_wording
                        INTO v_clause
                        FROM gin_clause
