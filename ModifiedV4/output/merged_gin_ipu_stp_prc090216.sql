```sql
PROCEDURE gin_ipu_stp_prc090216 (
    v_batchno        IN   NUMBER,
...
BEGIN
    ...
    FOR pol_cur_rec IN pol_cur LOOP
        IF pol_cur_rec.pol_policy_status = 'NB' THEN
        ...
        v_wef := v_wef_date;
        BEGIN
            IF v_pol_instal_wet != pol_cur_rec.pol_paid_to_date THEN
                UPDATE gin_policies
                SET
                    pol_paid_to_date = v_pol_instal_wet
                WHERE
                    pol_batch_no = v_batchno;
        ...
        v_ipu_prorata := 'P';
        IF v_wef_date NOT BETWEEN pol_cur_rec.pol_policy_cover_from AND pol_cur_rec.pol_policy_cover_to THEN
            raise_error (
        ...
        IF v_wet_date NOT BETWEEN pol_cur_rec.pol_policy_cover_from AND pol_cur_rec.pol_policy_cover_to THEN
            raise_error (
                'THE RISK COVER DATES PROVIDED MUST BE WITHIN THE POLICY COVER PERIODS. '
                || pol_cur_rec.pol_policy_cover_from
                || ' TO '
