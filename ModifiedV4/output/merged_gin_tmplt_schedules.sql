...
IS
    ...
    IS
        ...
        FROM
            gin_web_motor_sch
        WHERE
            mps_ipu_code = v_web_ipu_code;
BEGIN
    FOR rsks_cur_rec IN rsks_cur LOOP
        FOR sched_cur_rec IN sched_cur LOOP
            IF sched_cur_rec.screen_name = 'UMOTORP' THEN
                BEGIN
                    SELECT COUNT (1)
                      INTO v_cnt
                      FROM gin_motor_private_sch
                     WHERE mps_ipu_code = TO_NUMBER (v_new_ipu_code);
                      --Added logging from Geminia
                ...
                ELSE
                    ...
                    END;
                END IF;
            ELSIF sched_cur_rec.screen_name = 'UMOTCYC' THEN
                BEGIN
                    SELECT COUNT (1)
                      INTO v_cnt
                ...
                ELSE
                    ...
                    END;
                END IF;
            ELSIF sched_cur_rec.screen_name IN ( 'UMOTCOM', 'UMOTPSV' ) THEN
                BEGIN
                    SELECT COUNT (1)
                      INTO v_cnt
