...
BEGIN
    ...
    THEN
        ...
        EXCEPTION
            WHEN OTHERS
            THEN
                v_err := 'Error occurred on inserting facre details ...' || SQLERRM (SQLCODE);
                RETURN;
        END;
    ELSIF v_action = 'E'
    THEN
        BEGIN
            UPDATE gin_facre_in_dtls
               SET fid_cede_comp_policy_no = NVL (v_ced_pol_no, fid_cede_comp_policy_no),
                   ...
                   fid_reins_term_to     = NVL (v_rein_terms_to, fid_reins_term_to),
                   fid_cede_comp_gross_ret =
